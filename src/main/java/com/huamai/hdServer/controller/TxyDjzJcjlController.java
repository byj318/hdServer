 ...
 */
@Controller
@RequestMapping("/txydjzjcjl")
public class TxyDjzJcjlController {
	/**
	 * 进入透析液电解质监测记录页面
	 * 
	 * @param model
	...
	public String view(Model model, @PathVariable(name = "courtyardArea") String courtyardArea) {
		...
		model.addAttribute("title", "透析液电解质监测记录");
		model.addAttribute("nowYear", LocalDate.now().getYear());
		return "txydjzjcjl/list";
	}

	@Autowired
	private TxyDjzJcjlService txyDjzJcjlService;
	...
	public Object listPage(Integer pageIndex, Integer pageSize, String startTime, String endTime,
		...
		return txyDjzJcjlService.listAll(pageIndex, pageSize, startTime, endTime, courtyardArea);
	}
	...
	public Object deleteData(@PathVariable(name = "ids") String ids) {
		...
		try {
			txyDjzJcjlService.deleteDate(ids);
	...
	@PostMapping
	@ResponseBody
	public Object insertData(TxyDjzJcjl txyDjzJcjl) {
		ResponseVO responseVO = new ResponseVO();
		try {
			txyDjzJcjlService.insertData(txyDjzJcjl);
			responseVO.setSuccess(true);
	...
	@PutMapping
	@ResponseBody
	public Object updateData(TxyDjzJcjl txyDjzJcjl) {
		ResponseVO responseVO = new ResponseVO();
		try {
			txyDjzJcjlService.updateData(txyDjzJcjl);
			responseVO.setSuccess(true);
	...
	public void exportExcel(HttpServletRequest request, HttpServletResponse response, String startTime, String endTime,
			String courtyardArea) {
		txyDjzJcjlService.exportExcel(request, response, startTime, endTime, courtyardArea);
	}
}
